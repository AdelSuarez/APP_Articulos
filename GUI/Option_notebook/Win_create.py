...
class Win_create(ttk.Frame):
	def __init__(self, *args, **kwargs):
		...
		tk.Label(self.frame_create, text='Nombre: ', bg=config.BACKGROUND).grid(row=1, column=0, sticky=tk.E)		
		self.name_create = ttk.Entry(self.frame_create)
		self.name_create.grid(row=1, column=1, pady=4)

		tk.Label(self.frame_create, text='Precio: ', bg=config.BACKGROUND).grid(row=2, column=0, sticky=tk.E)
		self.price_create = ttk.Entry(self.frame_create)
		self.price_create.grid(row=2, column=1, pady=4)

		ttk.Button(self.frame_create, text='Confirmar', command=lambda:Create(self.name_create, self.price_create, self.message)).grid(row=3, column=0, columnspan=2, pady=4)
